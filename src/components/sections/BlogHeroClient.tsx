...
type BlogHeroClientProps = {
    stats: SerializableHeroStat[];
};
...
export function BlogHeroClient({ stats }: BlogHeroClientProps) {
        ...
        <section className="relative min-h-screen flex flex-col overflow-hidden bg-gradient-to-br from-background via-primary/5 to-hexo-blue-light/10 max-sm:mt-10">
            ...
            <div className="container mx-auto px-4 relative z-10 flex-1 flex items-center justify-center">
                ...
                >
                    ...
                    {/* Stats Grid - maintenant dynamiques */}
                    <motion.div
                        variants={containerVariants}
                        className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 lg:gap-8 max-w-4xl mx-auto"
                    >
                        {stats.map((stat, index) => {
                            // Sélection du composant d'icône basé sur l'ID
                            const IconComponent =
                                stat.iconId === 'FileText'
                                    ? FileText
                                    : stat.iconId === 'Grid3X3'
                                      ? Grid3X3
                                      : stat.iconId === 'Lightbulb'
                                        ? Lightbulb
                                        : stat.iconId === 'BookOpen'
                                          ? BookOpen
                                          : FileText; // Fallback
