...
export function ContactForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Envoi vers l'API (les fichiers sont déjà uploadés avec leurs URLs)
    ...
    // État pour les URLs des fichiers uploadés vers Vercel Blob
    ...
    const handleFilesUploaded = (results: UploadResult[]) => {
        ...
        // Mettre à jour les fichiers dans le formData pour l'email
        const fileInfos = successfulUploads.map((file) => ({
            name: file.filename,
            size: file.size,
            type: file.type,
            url: file.url,
        ...
        setFormData((prev) => ({
            ...prev,
            files: [...prev.files, ...fileInfos].slice(0, 5),
        }));
    };
